...
- static 멤버 변수는 사용할 수 없으나, `static final`은 사용 가능
  ...
  - 그럼 상수는 어떻게 가능것인가 하면 상수는 상수풀에 저장이되는 고정값으로 여러 객체에서 접근을 해서 사용을해도 위와 같은 문제가 발생 하지 않는다
...
- 외부 클래스의 private을 포함한 모든 멤버에 접근 가능
  ...
  - 내부클래스의 내부에서 외부클래스의 객체를 생성해서 그 객체를 통해 멤버변수에 접근이가능하다
...
- 정적 변수는 가질 수 없으나, static final 변수(상수)는 가질 수 있음
  ...
  - 클래스자체가 스택메모리에 생성되기 때문에 생명주기가 로컬영역과 함께한다. 그러나 static 변수는 클래스영역에 생성되어야 하는 변수기 때문에 로컬내부 클래스에서는 생성할 수 가 없다
...
- 로컬 영역의 변수 다음과 같은 사항을 따름
  ...
  - 로컬 내부 클래스의 객체는 힙메모리의 형성이 되지만 그 멤버 변수의 정보를 가지고 있는 클래스는 => 이부분 보류.. 스택영역에 생성됬다가 이미 사라진 상태이기 때문에 멤버변수를 가질 수 없고 상수풀에 저장되는 상수만 가질 수 있다.
...
- 로컬 내부 클래스와 동일하나, 이름이 없어 선언 즉시 한 번 사용 가능하다

  - 클래스를 따로 선언하지 않고 메서드의 구현과 함께 사용한다 => 기존의 방식과는 많이 다른 접근
  - 예제에서 1 => 2 => 3 순으로 일반적인 클래스와 메서드 구현이 생략되어져 가는 과정을 보여준다
    - Foo => InnerClass = >  익명클래스로 생략되어져 가는 과정을 이해해보자

- 추상 클래스나 인터페이스의 구현에 많이 사용

  - 인터페이스를 구현한 클래스의 객체를 사용한는 것이 아닌 인터페이스를 new해서 사용하면 해당 인터페이스를 구현한 익명의 1회용 객체를 생성해줌
  - 그것을 바로 밑에서 구현해주게 되면 익명의 클래스를 사용한 것이 됨
