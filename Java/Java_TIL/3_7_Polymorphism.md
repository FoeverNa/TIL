...
- 가상메서드란 실행 시점에 호출된 객체의 타입을 보고 어떤 메소드가 호출 될 것인지 결정하는 것 

  - 부모 클래스 타입으로 자녀클래스의 객체를 생성한 경우 둘 중 어떤 메서드가 사용될지 모르는 상황이기 때문에 가상메서드를 호출해서 실행 시점에서 객체의 타입을 보고 어떤 메서드를 실행할지 결정할지를 결정한다는 뜻이다

  - 이때 가상메서드를 불러온다는 의미로 가상메서드 콜이라고 한다?

  - 동적바인딩이 일어나는 거네?

    

- virtuals method는 부모메서드를 뜻하고 call된 샹황은 오버라이드된 메서드가 실행되는 것을 의미(강사님이 설명해준것)
