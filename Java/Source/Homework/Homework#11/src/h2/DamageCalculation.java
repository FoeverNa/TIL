...
class Player {
    ...
    double calcDamage(){
        Function<Item, Integer> usedItem = x ->{
            int sum =0;
            for (Item item : items){
              if(item==x)sum++;
          }
        ...
        return (weaponDamage+(usedItem.apply(Item.MUSHROOM)*20))
               * (1 + (usedItem.apply(Item.BLACK_POTION)*0.1))
                + (usedItem.apply(Item.WHITE_POTION)*200);
    }
